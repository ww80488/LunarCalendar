...
function saveBirthdays(o){
  try{localStorage.setItem(BK,JSON.stringify(o));}catch(e){}
}
...
function getUpcomingBirthdays(limit){
  limit=limit||3;
  var now=new Date(),bdays=loadBirthdays(),upcoming=[];
  for(var key in bdays){
    var b=bdays[key];
...
/* ── ZODIAC COMPATIBILITY ── */
var ZODIAC_COMPAT={
  "Rat":{el:"Water",fix:"Yang",best:["Ox","Dragon","Monkey"],avoid:["Horse","Rabbit","Goat"],traits:"Quick-witted, resourceful, ambitious"},
  "Ox":{el:"Earth",fix:"Yin",best:["Rat","Snake","Rooster"],avoid:[\
